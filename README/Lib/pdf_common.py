...
def generate_outline_str(bookmarks, indent=0):
	...
	for i, (label, [page, _, _], children) in enumerate(bookmarks, 1):
		...
		#
		str_ = '<li>'+('%s <b>%d</b>' % ( _label, (page-1)))+'</li>'
		#
		if _label == "Cover" or _label == "Contents":
			#
			str_ = ''
			#str_ = '<li>'+('%s' % ( label))+'</li>'
			#
...
def add_position_tuple(self,h_lists, tup_list):
	...
	def iterr(f_list, _l, _t, c_c):
		...
		for x in _t:
			#
			cur_chain = cur_chain + ' / ' + x[0].split(":")[0]
			#
			if x[0] == f_list[-1]:
				#
				d = list(x)
	...
	for page_number, page in enumerate(self.pages):
		...
		for level, label, (point_x, point_y) in page.bookmarks:
			...
			if h_level == 2 or h_level == 3: # only h2 and h3 are considered labels
				#
				labels.append([_label.split(' / '),(page_number, point_x, point_y)])
				#
			#
		#
	#
...
def add_intro(doc, new_data, new_list, css_file, css_file_contents, css_file_cover, version, up_dir, title, gen_date):
	...
	#
	#
	table_of_contents_document = HTML(string=TOC_str).render(stylesheets=[css_file_contents], enable_hinting=False)
	...
	#
#
def make_pdf(new_data, new_list, css_file, css_file_contents, css_file_cover, version, up_dir, title, gen_date, lang):
	#
	write_data = new_data.replace("assets/media", "README/assets/media")
	#
	doc = HTML(string=write_data, base_url=os.path.join(up_dir)).render(stylesheets=[css_file], enable_hinting=False)
	doc.make_bookmark_tree('convert')
	#
	data = make_bookmark_tree_b(doc,'last')
	#
	output = os.path.join(up_dir, "README_"+lang+".pdf")
	#
