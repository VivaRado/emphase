	...
	for file in sorted(glob.glob(os.path.join(dir_path,"*.md"))):
		...
		if "README.md" not in file and check_name not in file:
			...
			if f_name not in added:
				...
				do_open = True
				#
				if "V0" in f_name_ or "V1" in f_name_: # prevent non matching volumes from being included but allow levelless files normally.
					#
					if f_name_.split('_')[1] != volumes[0]:
						#
						added.remove(f_name)
						#
						do_open = False
...
def make_default_github_md(dir_path, up_dir, name, volumes):
	...
	print (os.path.join(up_dir, name))
	...
	with open(new_README,"w") as outfile:
		...
		for x in matched_lines:
			...
			#
		#
		data_c = data_b.replace("(assets/", "(README/assets/")
		#
...
def md_to_html(dir_path, md_name):
	...
	with open(os.path.join(dir_path,md_name), 'r') as in_file: 
		col_html = markdown2.markdown(in_file.read(), extras=["fenced-code-blocks","markdown-in-html", "tables", "strike","target-blank-links"])
